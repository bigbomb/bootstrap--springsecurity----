...
<script type="text/javascript">
	...
	$(function() {
		grid = window.lanyuan.ui.lyGrid({
					...
					jsonUrl : '${pageContext.request.contextPath}/background/dicType/query.html',
					checkbox : true
				});
		...
		$("#add").click("click", function() {//绑定查询按扭
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/dicType/addUI.html',
				title : "增加字典类型",
		...
		$("#editView").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/dicType/editUI.html?id='+cbox,
				title : "修改字典类型",
				isHidden : false
			});
		});
			...
			parent.$.ligerDialog.confirm('删除后不能恢复，确定删除吗？', function(confirm) {
				if (confirm) {
					$.ajax({
					    ...
					    url: rootPath + '/background/dicType/deleteById.html', //要访问的后台地址
	...
	<div class="divBody">
		<div class="search">
			<form name="fenye" id="fenye">
				名称：<input type="text" name=dicTypeName value="${param.name}"
